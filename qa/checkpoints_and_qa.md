...
31 checks. 28 are mechanically verifiable and are
...
| QA-06 | Every cross-reference resolves to something that exists. | Collect all headings from each Markdown source, collect every reference of the form 'Annex X' or 'section N', then assert each reference matches a heading in the same document. Extend to the compliance matrix mapping column. Implemented as CON-01 and CON-06; both pass on the current build. |
| QA-07 | No pricing appears anywhere in the technical proposal. | Search the technical PDF for EY pricing constructs, being day rates, fee lines, totals and discounts for this assignment. Benchmark figures quoted as evidence are permitted, and the RFP's own requirement for the scope and magnitude of past assignments depends on them. CI evaluates the financial proposal separately and only for technically qualified bidders, so price leaking into the technical file is a live compliance risk. Not currently checked. |
...
| QA-12 | The all-inclusive total is exactly US$40,000 and no figure anywhere exceeds the ceiling. | scripts/qa.py ARI-02 flags any figure above the ceiling. Add a hard failure on any total above 40,000, plus a warning on any deviation from the price approved at CP-5, since the RFP sets a maximum rather than a target and a total below it must be deliberate rather than an arithmetic slip. |
...
| QA-15 | Day counts reconcile across the fee table, the effort-by-deliverable table and the effort-by-week annex. | Sum the Days column in each of the three tables and assert they agree with the fee table's total rather than with a hard-coded number, so the check survives a change to the effort model. Implemented as ARI-07. |
| QA-16 | All cost information is expressed in USD and no other currency appears in the cost proposal. | scripts/qa.py ARI-05 searches the cost source for ZAR, rand figures, EUR, euro and pound symbols. The check is scoped to the cost proposal because the RFP's USD rule applies to cost information. Benchmark figures elsewhere may carry other currencies from published sources. |
...
| QA-29 | All seven of CI's stated design aims are addressed in the technical proposal. | Assert each of the seven aims appears in the option-design method or the Annex F rating matrix. Implemented as INV-03. |
| QA-30 | All eight minimum skills from RFP section 3 are addressed with evidence or a placeholder. | Assert each of the eight skills appears in Annex C with either evidence or a [[TO CONFIRM]]. Implemented as INV-04. |
| QA-31 | No EY credential, client reference, staff name or rate is asserted rather than placeheld. | Assert the only person named in the staffing and reference tables is Michael Harris, and that no EY credential or commercial claim appears without a placeholder beside it. Implemented as INV-01 and INV-02. |
